...
static RET_DEFINE ExTransferContactAsset(unsigned char* ipara, void * pVmEvn) {
	CVmRunEvn *pVmRunEvn = (CVmRunEvn *)pVmEvn;
	vector<std::shared_ptr < vector<unsigned char> > > retdata;

    if(!GetData(ipara,retdata) ||retdata.size() != 1 || retdata.at(0).get()->size() != 34)
    {
    	return RetFalse(string(__FUNCTION__)+"para  err !");
    }
	...
	string honghuoaddr = SendKeyID.ToAddress();
	sendkey.assign(honghuoaddr.c_str(),honghuoaddr.c_str()+honghuoaddr.length());

	recvkey.assign(retdata.at(0).get()->begin(), retdata.at(0).get()->end());

	std::string recvaddr( recvkey.begin(), recvkey.end() );
	if(honghuoaddr == recvaddr)
	{
	...
	std::shared_ptr<CAppUserAccout> recvAccout = std::make_shared<CAppUserAccout>();
	...
	if (!pContractScript->GetScriptAcc(script, recvkey, *recvAccout.get())) {
		recvAccout = std::make_shared<CAppUserAccout>(recvkey);
	}

	recvAccout.get()->setLlValues(recvAccout.get()->getllValues() + temp.get()->getllValues());
...
static RET_DEFINE ExTransferSomeAsset(unsigned char* ipara, void * pVmEvn) {
	CVmRunEvn *pVmRunEvn = (CVmRunEvn *)pVmEvn;
	vector<std::shared_ptr < vector<unsigned char> > > retdata;
	unsigned int Size(0);
    ...
    if(!GetData(ipara,retdata) ||retdata.size() != 2 || retdata.at(1).get()->size() != 34
    	|| retdata.at(0).get()->size() != Size)
    {
			return RetFalse(string(__FUNCTION__)+"para err !");
    }
	...
	recvkey.assign(retdata.at(1).get()->begin(), retdata.at(1).get()->end());
	...
	else
	{
		...
		else
		{
			...
			recvAccout.get()->setLlValues(recvAccout.get()->getllValues() + uTransferMoney);
		}
	}
...
enum CALL_API_FUN {
	...
	TRANSFER_CONTACT_ASSET_FUN,
	TRANSFER_SOME_ASSET_FUN
};
...
const static struct __MapExterFun FunMap[] = { //
		...
		{TRANSFER_CONTACT_ASSET_FUN,ExTransferContactAsset},
		{TRANSFER_SOME_ASSET_FUN,ExTransferSomeAsset}
		};
